...
{
	numbers.Add((Index: index, Value: int.Parse(line)));
	index++;
}
var multiplier = numbers.Select(n => n.Value).Max() / numbers.Count() + 1;
...
void MoveLeft(int start, int count)
{
	...
	{
		var indexTo = (numbers.Count() * multiplier + start - i) % numbers.Count();
		var indexFrom = (numbers.Count() * multiplier + start - i - 1) % numbers.Count();
		numbers[indexTo] = numbers[indexFrom];
	}
	numbers[(numbers.Count() * multiplier + start - count) % numbers.Count()] = number;
}
...
void MoveRight(int start, int count)
{
	...
	numbers[(numbers.Count() + start + count) % numbers.Count()] = number;
}